...
class CancelledMultipleRoom extends ItineraryHotelItemBaseSpec{
	@Shared
	...
	@Shared
	HotelSearchData hotelData = new HotelSearchData("LeesBoutique-SITUJ15XMLAuto-MultiRoom")
	@Shared
	HotelTransferTestResultData resultData = new HotelTransferTestResultData("LeesBoutique-SITUJ15XMLAuto-MultiRoom",hotelData)
	...
	def "05 : Verify Booked Item Screen Results   "() {
			...
			verifyBookedItemScreenMultiRoomCancel(hotelData, resultData)
			}
	
	def "06 : Verify Cancel Item Screen Results   "() {
		
			given: "User is logged in and present on Cancel Items Screen"
			verifyCancelBookedItemsScreenresults(hotelData, resultData)
	}
